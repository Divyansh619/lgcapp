...
const ManagingComitee = () => {
  ...
  const FetchData = () => {
    ...
    fetch("http://3.111.3.33/api/LGCfrontend/MemberCommitteeList", requestOptions)
    ...
    <ScrollView>
      {isLoading ? (
        ...
        {data ? <View style={styles.container}>
          ...
          {data.map((item, index) => {
            return <ProfileCard
              key={index} imageSource={item.Image}
              name={item.Name}
...
export default ManagingComitee;
